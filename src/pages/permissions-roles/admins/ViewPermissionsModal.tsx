...
} from "@/api/permissions-roles/admins/roles/roles.hooks";
...
export function ViewPermissionsModal({ role, open, onOpenChange }: Props) {
  const { data, isPending } = useShowRole(role?.id!);
  const permissions = data ? data.permissions : [];
  const [togglingId, setTogglingId] = useState<number | null>(null);
  const { mutate: togglePermission } = useToggleRolePermission();
