...
import { useBranchesShowRole, useToggleBranchesRolePermission } from "@/api/permissions-roles/branches/branches.hooks";
...
export function BranchesPermissionsModal({ role, open, onOpenChange }: Props) {
    const { data, isPending } = useBranchesShowRole(role?.id || 0);
    const permissions = data ? data.permissions : [];
    const [togglingId, setTogglingId] = useState<number | null>(null);
    const { mutate: togglePermission } = useToggleBranchesRolePermission();
