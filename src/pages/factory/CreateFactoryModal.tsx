...
import { useCreateFactoryMutationOptions } from "@/api/v2/factories/factories.hooks";
import { TCreateFactoryRequest } from "@/api/v2/factories/factories.types";
...
export function CreateFactoryModal({ open, onOpenChange }: Props) {
  const { mutate: createFactory, isPending } = useMutation(
    useCreateFactoryMutationOptions()
  );
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    const requestData: TCreateFactoryRequest = {
    ...
    createFactory(requestData, {
      onSuccess: () => {
        toast.success("تم إنشاء المصنع بنجاح", {
          description: "تمت إضافة المصنع بنجاح للنظام.",
        });
        form.reset();
        onOpenChange(false);
      },
      onError: (error) => {
        toast.error("حدث خطأ أثناء إنشاء المصنع", {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-hide">
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            املأ البيانات لإضافة مصنع جديد للنظام.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="المصنع الرئيسي" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
