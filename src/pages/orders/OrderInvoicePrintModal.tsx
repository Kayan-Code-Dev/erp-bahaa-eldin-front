...
const INVOICE_PRINT_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .invoice-print-header-right .invoice-header-number { font-size: 28px; font-weight: 700; letter-spacing: -0.025em; }
  ...
  .invoice-print-title { font-size: 24px; font-weight: bold; text-align: center; margin-bottom: 8px; color: #1f2937; }
  ...
  .invoice-print-table { width: 100%; border-collapse: collapse; margin-bottom: 24px; table-layout: fixed; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; page-break-inside: avoid; }
  .invoice-print-table td, .invoice-print-table th { padding: 14px 16px; border-bottom: 1px solid #f3f4f6; }
  .invoice-print-td { padding: 14px 16px; border-bottom: 1px solid #f3f4f6; }
  .invoice-print-th { text-align: center; font-weight: 600; font-size: 13px; padding: 14px 16px; }
  .invoice-print-thead-row th { background: #7a6349; color: #fff; }
  .invoice-print-row-even td { background: rgba(249,250,251,0.8); }
  .invoice-print-td-center { text-align: center; font-size: 14px; }
  .invoice-print-td-bold { font-weight: 600; }
  .invoice-print-block { margin-bottom: 24px; page-break-inside: avoid; }
  .invoice-print-notes-box { font-size: 14px; border: 1px solid #e5e7eb; padding: 12px 16px; min-height: 52px; border-radius: 8px; background: rgba(249,250,251,0.5); color: #374151; }
  .invoice-print-rules-row { display: flex; gap: 24px; margin-bottom: 24px; flex-wrap: wrap; page-break-inside: avoid; }
  .invoice-print-rules-text { flex: 1; min-width: 240px; }
  .invoice-print-rules-p { font-size: 13px; line-height: 1.6; color: #4b5563; }
  .invoice-print-totals-wrap { flex-shrink: 0; width: 224px; border-radius: 12px; overflow: hidden; border: 1px solid #e5e7eb; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
  .invoice-print-totals-title { padding: 10px 16px; text-align: center; color: #fff !important; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; background: #907457 !important; background-color: #907457 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .invoice-print-totals-table { width: 100%; border-collapse: collapse; }
  .invoice-print-totals-row { border-bottom: 1px solid #f3f4f6; }
  .invoice-print-totals-row:last-child { border-bottom: none; }
  .invoice-print-totals-label { padding: 12px 16px; font-size: 13px; font-weight: 600; color: #4b5563; background: rgba(249,250,251,0.8); }
  .invoice-print-totals-value { padding: 12px 16px; text-align: right; font-variant-numeric: tabular-nums; font-weight: 600; color: #111827; }
  .invoice-print-totals-table .invoice-print-totals-value:first-of-type { font-size: 15px; font-weight: 700; }
  .invoice-print-signature { display: flex; justify-content: flex-end; margin-top: 40px; padding-top: 24px; border-top: 1px solid #e5e7eb; page-break-inside: avoid; }
  ...
  @media print {
    .invoice-print-header, .invoice-print-footer, .invoice-print-totals-title { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .invoice-print-thead-row th, .invoice-print-row-even td { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
